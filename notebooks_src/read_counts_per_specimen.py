...
num_reads_per_specimen_isotype_df_pivot.to_csv(
    config.paths.base_output_dir / "reads_per_specimen_and_isotype.tsv", sep="\t"
)
...
num_unique_vdj_per_specimen_isotype_df_pivot.to_csv(
    config.paths.base_output_dir / "unique_vdj_per_specimen_and_isotype.tsv", sep="\t"
)
...
# %% [markdown]
# # Plots of all specimens: disabled
#
# - slow!
# - exceeds Matplotlib figure size limits as our dataset gets bigger
...
#         g.fig,
#         config.paths.high_res_outputs_dir_base
#         / f"repertoire_read_counts.unique_vdj_sequences.{slugify(disease)}.png",
#         dpi=72,
#     )
#     plt.close(g.fig)
...
#         g.fig,
#         config.paths.high_res_outputs_dir_base
#         / f"repertoire_read_counts.total_reads.{slugify(disease)}.png",
#         dpi=72,
#     )
#     plt.close(g.fig)
...
genetools.plots.savefig(
    g.fig,
    config.paths.base_output_dir
    / "repertoire_read_counts.total_reads.by_disease.shared_ylims.png",
    dpi=100,
)
...
genetools.plots.savefig(
    g.fig,
    config.paths.base_output_dir / "repertoire_read_counts.total_reads.by_disease.png",
    dpi=100,
)
...
genetools.plots.savefig(
    g.fig,
    config.paths.base_output_dir
    / "repertoire_read_counts.unique_vdj_sequences.by_disease.shared_ylims.png",
    dpi=100,
)
...
genetools.plots.savefig(
    g.fig,
    config.paths.base_output_dir
    / "repertoire_read_counts.unique_vdj_sequences.by_disease.png",
    dpi=100,
)
