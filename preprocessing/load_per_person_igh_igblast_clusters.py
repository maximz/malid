...
def main(arguments):
    ...
    clones_exists  = engine.has_table('igh_clones_%s' % args.participant_label,schema=args.schema)
    members_exists = engine.has_table('igh_clone_members_%s' % args.participant_label, schema=args.schema)
    if clones_exists or members_exists:
        if clones_exists:
            logging.error('clones table igh_clones_%s already exists' % args.participant_label)
        if members_exists:
            logging.error('clone member table igh_clone_members_%s already exists' % args.participant_label)
        sys.exit(10)
    ...
    clones_table =  Table('igh_clones_%s' % args.participant_label, meta,
            Column('igh_clone_id', Integer, ForeignKey('%s.%s' % (args.schema, part_igh_table.c.part_igh_id)), primary_key=True),
            schema=args.schema)
    logging.info('creating clones table igh_clones_%s' % args.participant_label)
    clones_table.create()
    ...
    with out_constraints(engine, clones_table):
        ...
        with batched(args.batch_size, lambda batch: engine.execute(clones_insert, batch)) as batch:
            for ident in clone_ids:
                batch.add({'igh_clone_id': ident})

    members_table = Table('igh_clone_members_%s' % args.participant_label, meta,
            Column('igh_clone_id', Integer, ForeignKey('%s.%s' % (args.schema, part_igh_table.c.part_igh_id)), nullable=False, primary_key=True),
            Column('part_igh_id', Integer, ForeignKey('%s.%s' % (args.schema, part_igh_table.c.part_igh_id)), nullable=False, primary_key=True),
            schema=args.schema)
    logging.info('creating clone member table igh_clone_members_%s' % args.participant_label,)
